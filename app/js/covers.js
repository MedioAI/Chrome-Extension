...
const covers = {
  ...
  init: () => {
    ...
    const next = document.querySelector('#medioNext');
    next.addEventListener('click', e => {
      covers.currentPage++;
      if (covers.currentPage > covers.totalPages) {
        covers.currentPage = covers.totalPages;
      }
      document.getElementById('covers').innerHTML = ''; 
      covers.load(); 
    });

    const prev = document.querySelector('#medioPrev');
    prev.addEventListener('click', e => {
      covers.currentPage--;
      if (covers.currentPage < 1) {
        covers.currentPage = 1;
      }
      document.getElementById('covers').innerHTML = ''; 
      covers.load(); 
    });
  ...
  load: () => {
    ...
    chrome.storage.local.get('medioAICovers', async data => {
      ...
      if (allCovers) {
        ...
        document.querySelector('#medioPageCount').textContent = `1 of ${totalPages}`
        ...
        let currentPageCovers = allCovers
        let currentPageNumber  = covers.currentPage
        if (currentPageCovers.length > 12) {
          if (covers.currentPage > totalPages) {
            currentPageNumber = totalPages
          }
          currentPageCovers = allCovers.slice((currentPageNumber - 1) * 12, currentPageNumber * 12)
          document.querySelector('#medioPageCount').textContent = `${currentPageNumber} of ${totalPages}`
        }


        for (let index = 0; index < currentPageCovers.length; index++) {
          const element = currentPageCovers[index]
          covers.build(element, index)
        }
    ...
    chrome.storage.local.get('medioAICovers', async data => {
      ...
      modal.innerHTML = /* html */ `
        <div id="coverPreview" class="relative w-full max-w-2xl bg-black rounded-lg p-4">
          ...
          <h3 class="text-2xl">${cover.prompt}</h3>
        </div>
